function Datatx=transmitter_non_FSS(Gen_data,Numsymb,wordsize,NumCarr,guardtype,...
                                guardtime,Num_pilot,proc_gain,Num_Tx,Num_User)
% Gen_data=[20,64,2,4]の0or1,Numsymb=20,wordsize=2,NumCarr=64,guardtype=2,guardtime=16,Num_pilot=2,proc_gain=64,Num_Tx=2
% Gen_data=[20,64,2,1:2]はアンテナ1,2からの成分
% Gen_data=[20,64,2,3:4]はアンテナ3,4からの成分
%*******************************************************************************         
...
proc_gain_block=NumCarr/proc_gain;% 64/64=1 → 1 blockしか使用しないので、
spread=hadamard(proc_gain);% [64,64]のアダマール行列

%*********************
% Make a pilot signal
%*********************
Pilot=zeros(Num_pilot,NumCarr,Num_Tx);% [4,64,4]の1 ⇒ パイロット信号（受信機での信号特定に使用）
...
elseif wordsize==2% QPSK ⇒ [20,64,2] 2by2 ※ Data = [20,64,2,2]
    for transmit=1:Num_Tx
        ...
        % Datatx(:,:,1) → 基地局の送信アンテナ1
        % Datatx(:,:,2) → 基地局の送信アンテナ2
        % Datatx(:,:,3) → 基地局の送信アンテナ3
        % Datatx(:,:,4) → 基地局の送信アンテナ4
    end
...
Datatx(Num_pilot+1:Numsymb+Num_pilot,:,:)=Datatx1/Num_Tx;% [20,64,4]のデータ
% Datatx(24,64,4)のデータが返る。
