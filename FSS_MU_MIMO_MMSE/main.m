...
rep=200;%Number of loop times ⇒　任意の回数ループする（シミュレーション回数）
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
	  ...
	  for r = 1:rep % 1～100 ⇒ 各々のEb/Noに対して任意の回数シミュレーションする
        ...
        for i=1:Num_Rx % 1～2
            for j=1:Num_Tx % 1～2 Signal_disは後で改善する ⇒ ただデータを保存してるだけなので、Fadingに直接の影響はない。
                [OutSignal1(:,:,j),Signal_dis1(j), Fading1(:,:,j)]=channel_only(BaseSignal(:,:,j),NormMulti,MultiNo,...
                                            Transrate,Doppler,Delay,SNR,com_delay);% channel_only([68,22,2],[7,1],7,1000,4,2,7,0)
                OutSignal(:,:,i)=OutSignal(:,:,i)+OutSignal1(:,:,j);
                Signal_dis(i)=Signal_dis(i)+Signal_dis(j);
        ...
        for receive=1:Num_Rx
             [TimeSignal(:,:,receive),noise(:,:,receive)]=awgn(OutSignal(:,:,receive),Signal_dis(receive),MultiNo,Doppler,SNR,Num_sym,...
        ...
        [Datarx,Datarx_hd]=receiver_FSS(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                    guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_Tx,Num_Rx);% [[1,22*68,2],64,[1,64],1,2,4,20,2,4,64,2,2]
