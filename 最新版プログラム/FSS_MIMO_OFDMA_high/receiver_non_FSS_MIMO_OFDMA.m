...
for fsc=1:size(user_index,2)% 1～2
    for fscb=1:size(user_index,1)% 1～4
        for b=1:block_num_per_user
            data_faded_final(:,(channel_ranking(fscb,b,fsc)-1)*proc_gain+1:channel_ranking(fscb,b,fsc)*proc_gain,fsc)=data_faded1(:,(channel_ranking(fscb,b,fsc)-1)*proc_gain+1:channel_ranking(fscb,b,fsc)*proc_gain,fsc,user_index(fscb,fsc));
        end
%         data_faded_final(:,(channel_ranking(fscb,1,fsc)-1)*proc_gain+1:channel_ranking(fscb,1,fsc)*proc_gain,fsc)=data_faded1(:,(channel_ranking(fscb,1,fsc)-1)*proc_gain+1:channel_ranking(fscb,1,fsc)*proc_gain,fsc,user_index(fscb,fsc));
%         data_faded_final(:,(channel_ranking(fscb,2,fsc)-1)*proc_gain+1:channel_ranking(fscb,2,fsc)*proc_gain,fsc)=data_faded1(:,(channel_ranking(fscb,2,fsc)-1)*proc_gain+1:channel_ranking(fscb,2,fsc)*proc_gain,fsc,user_index(fscb,fsc));
    end
end
