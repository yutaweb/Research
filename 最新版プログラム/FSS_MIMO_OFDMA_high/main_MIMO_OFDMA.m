...
guardtype=2;
guardtime=16;
windowtype=3;% BPSK:1,QPSK:2,etc
CarrSpacing=1;
MultiNo=7;% マルチパス数
Transrate=20*10^6;
...
proc_gain=64; %OFDMAによって、4ユーザに割り当てるので16×16のマトリックス
...
block_num_per_user=16; %1 2,4,8,16,32,64
% proc_gain / block_num_per_user
...
for l = 1:NumSizes % 1
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
      ...
      for r = 1:rep % 1～100 ⇒ 各々のEb/Noに対して任意の回数シミュレーションする
        ...
        %% estimate channel
        [channel_ranking,user_index]=main_virtual_high(SNR,Doppler,com_delay,ifftsize,guardtype,guardtime,...
            MultiNo,Transrate,Delay,ww,Num_sym,NumCarr,Num_pilot,strength_len,carriers,proc_gain,...
            WordSizes,NumSizes,k,Num_Tx,Num_Rx,Num_User,channel_rand,r,block_num_per_user);
