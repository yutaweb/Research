...
if Doppler==0
   ...
   inv_H_m_resp=zeros(Num_Rx,Num_Rx,ifftsize);
   for i=1:ifftsize
        inv_H_m_resp(:,:,i)=inv(H_m_resp(:,:,i));
   end
   % ココでの計算は、data_faded1が[20,64,2]の状態を保ったまま
   data_faded=zeros(Num_sym,ifftsize,Num_Rx*Num_Tx);
   data_faded(:,:,1)=DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,1).*repmat(permute(inv_H_m_resp(1,1,:),[1 3 2]),Num_sym,1);
   data_faded(:,:,2)=DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,2).*repmat(permute(inv_H_m_resp(1,2,:),[1 3 2]),Num_sym,1);
   data_faded(:,:,3)=DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,1).*repmat(permute(inv_H_m_resp(2,1,:),[1 3 2]),Num_sym,1);
   data_faded(:,:,4)=DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,2).*repmat(permute(inv_H_m_resp(2,2,:),[1 3 2]),Num_sym,1);
   
   data_faded1=zeros(Num_sym,ifftsize,Num_Rx);
   data_faded1(:,:,1)=data_faded(:,:,1)+data_faded(:,:,2);
   data_faded1(:,:,2)=data_faded(:,:,3)+data_faded(:,:,4);
end
...
% disp(data_faded); ←QPSKなので、理想としては、0.7+0.7i etcに近い値になってくれないと困る。
