...
%h1*x1+h2'*x2+noise1
%x1*h1'+x2*h2'+noise2
gause_noise=Num_Tx/SNR.*eye(Num_Rx);
if Doppler==0
    data_faded1=received_signal(Num_pilot+1:Num_pilot+Num_sym,:); % [20,64,2]  
else
   
   DataCarriers1=permute(received_signal,[3 1 2]);%[2 20 64];
    ...
    for  k = 1:Num_sym % 1:20
       for cc=1:NumCarr % 1:64
          ...
          elseif wordsize==2
             ...
             X(k,cc,i)=data_faded1(k,cc,i)*sqrt(2); % [20,64,1～2]
