...
rep=50;%Number of loop times ⇒　任意の回数ループする（シミュレーション回数）
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
	  ...
	  for r = 1:rep % 1～100 ⇒ 各々のEb/Noに対して任意の回数シミュレーションする
        ...
        for receive=1:Num_Rx
             [TimeSignal(:,:,receive),snr]=awgn(OutSignal(:,:,receive),Signal_dis(receive),MultiNo,Doppler,SNR,Num_sym,...
        ...
        [Datarx,Datarx_hd]=receiver_non_FSS_MLD(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                    guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_Tx,Num_Rx,snr);% [[1,22*68,2],64,[1,64],1,2,4,20,2,4,64,2,2]
...
legend('FSS-MIMO(MMSE)','FSS-MIMO(FEC)');
xlabel('Eb/No [dB]');
ylabel('BER');
axis([0,30,10^(-5),10^(0)]);
grid;
