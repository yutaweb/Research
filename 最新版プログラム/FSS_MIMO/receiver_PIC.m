...
if Doppler==0
   ...
   % i > 0 
   % i回の周波数領域等化後に、IFFTを掛ける
   % 1st SIMO-MMSE
   data_faded_x1_no1=zeros(Num_sym,NumCarr,Num_Tx); % 送信アンテナ１からの信号
   data_faded_x2_no1=zeros(Num_sym,NumCarr,Num_Tx); % 送信アンテナ２からの信号
   
   data_faded_x1_no2=zeros(Num_sym,NumCarr); % 送信アンテナ１からの信号
   data_faded_x2_no2=zeros(Num_sym,NumCarr); % 送信アンテナ２からの信号
   % 受信アンテナダイバーシチ
   for transmit=1:Num_Tx
        data_faded_x1_no1(:,:,transmit)=data_faded1(:,:,1);
        data_faded_x2_no1(:,:,transmit)=data_faded1(:,:,2);
   end
   % [20,64,2] ※data_faded_x1_no1(:,:,1)とdata_faded_x1_no1(:,:,2)には同じ成分が入っている。
   
   % MMSE
   for i=1:Num_sym % 1～20
         for j=1:NumCarr % 1～64 
             data_faded_x1_no2(i,j)=H_m_resp(:,1,j)'/(H_m_resp(:,1,j)'*H_m_resp(:,1,j)+Noise_power1(1,j))*permute(data_faded_x1_no1(i,j,:),[3 2 1]);
             data_faded_x2_no2(i,j)=H_m_resp(:,2,j)'/(H_m_resp(:,2,j)'*H_m_resp(:,2,j)+Noise_power2(1,j))*permute(data_faded_x2_no1(i,j,:),[3 2 1]);
         end
   end
   
   data_faded_x=zeros(Num_sym,NumCarr,Num_Tx);
   data_faded_x(:,:,1)=data_faded_x1_no2;
   data_faded_x(:,:,2)=data_faded_x2_no2;
   % [20,64,2]
   ...
   for i=1:Num_Tx % 1～2
      BaseSignal(:,:,i)=(ifft(data_faded_x(:,:,i)'))*sqrt(NumCarr);% ⇒ [64,20,i]（Datatxで正規化の為に、sqrt(64)で割ったので、ifft部では、sqrt(64)を掛けている）
   end
