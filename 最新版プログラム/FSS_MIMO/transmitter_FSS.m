...
elseif wordsize==4% 16QAM
    for transmit=1:Num_Tx
        Datatx1(:,:,transmit)=((Data(:,:,1,transmit)*2+Data(:,:,2,transmit))+sqrt(-1).*(Data(:,:,3,transmit)*2+Data(:,:,4,transmit)))./sqrt(10);
    end
end
...
% Datatx(Num_pilot+1:Numsymb+Num_pilot,:,:)=permute(Datatx2,[1 2 4 3])/Num_Tx;% [20,64,2]のデータ
Datatx(Num_pilot+1:Numsymb+Num_pilot,:,:)=permute(Datatx2,[1 2 4 3]);% [20,64,2]のデータ
