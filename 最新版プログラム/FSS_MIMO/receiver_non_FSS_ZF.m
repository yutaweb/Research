...
received_signal_final=zeros(Num_sym,NumCarr,Num_Tx);
received_signal1=permute(received_signal,[3 1 2]);
for k=1:Num_sym % 1 ～ 20
    for cc=1:NumCarr % 1 ～ 64
        received_signal_final(k,cc,:)=inv(H_m_resp(:,:,cc))*received_signal1(:,k,cc);
    end
end
...
for i=1:Num_Rx
    for  k = 1:Num_sym % 1:20
       for cc=1:NumCarr % 1:64
          ...
          elseif wordsize==4
             X=received_signal_final(k,cc,i)*sqrt(10);
             Xi=real(X);
             Xq=imag(X);
             inst_dec1=Xi>0;
             Datarx_hd(k,cc,1,i)=inst_dec1;
             Datarx(k,cc,1,i)=-(Xi);
             if inst_dec1==0
                Xi=Xi+2; 
             else
             ...
             Datarx_hd(k,cc,2,i)=sign(Xi)==1;
             Datarx(k,cc,2,i)=-Xi;

             inst_dec2=Xq>0;
             Datarx_hd(k,cc,3,i)=inst_dec2;
             Datarx(k,cc,3,i)=-(Xq);
             if inst_dec2==0
                Xq=Xq+2; 
             else
