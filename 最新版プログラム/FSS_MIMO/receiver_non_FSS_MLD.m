...
h1_1_final=permute(H_m_resp(1,1,:),[1 3 2]);%h1
h1_2_final=permute(H_m_resp(1,2,:),[1 3 2]);%h2'
h2_1_final=permute(H_m_resp(2,1,:),[1 3 2]);%h1'
h2_2_final=permute(H_m_resp(2,2,:),[1 3 2]);%h2
        
%チャネル補償処理 ⇒ 周波数軸上
received_signal=DataCarriers(1+Num_pilot:numsymb,:,:);
%[20,64,2]
%h1*x1+h2'*x2+noise1,x1*h1'+x2*h2'+noise2
...
for k=1:Num_sym % k=1～20
   for cc=1:NumCarr % cc=1～62
      for i=1:size(x,2)
         for j=1:size(x,2)
            z1(i,j)=norm(received_signal(k,cc,1)-h1_1_final(1,cc).*x(i)-h1_2_final(1,cc).*x(j));  
            z2(i,j)=norm(received_signal(k,cc,2)-h2_1_final(1,cc).*x(i)-h2_2_final(1,cc).*x(j));    
         end
      end
      z=z1+z2;
      [MIN_G,index_G]=min(z);
      [MIN_L,index_L]=min(MIN_G);
      received_signal_final(k,cc,1)=x(index_G(index_L));
      received_signal_final(k,cc,2)=x(index_L);
...
for i=1:Num_Rx
    for  k = 1:Num_sym % 1:20
       for cc=1:NumCarr % 1:64
          ...
          elseif wordsize==4
             X=received_signal_final(k,cc)*sqrt(10);
             Xi=real(X);
             Xq=imag(X);
             inst_dec1=Xi>0;
