function [Datarx,Datarx_hd]=receiver(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                     guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_User)
...
for u=1:Num_User
    SymbLen(1,u)=length(TimeSignal(:,:,u))+guardtime;% 22*80+16 ⇒ 全体の信号にもガードインターバルを付与している
    TimeSignal(:,:,u)=TimeSignal(:,1:(SymbLen(1,u)-rem(SymbLen(1,u),ifftsize+guardtime)),u);% rem：余りを返す
    % rem(22*80+16,64+16)=16 ⇒ Symblenから16を引く。 TimeSignal=[1,22*80,4]
end
...
%***************************************************************************
symbwaves1=zeros(ifftsize+guardtime,numsymb,Num_User); % [80,22,4]
symbwaves=zeros(ifftsize,numsymb,Num_User); % [64,22,4]
if guardtype ~= 0 % guardtypeが0でないならば、1を返し、0ならば0を返す。
    for u=1:Num_User
        symbwaves1(:,:,u)=reshape(TimeSignal(:,:,u),ifftsize+guardtime,numsymb);% [80,22,4]
        symbwaves(:,:,u)=symbwaves1(guardtime+1:ifftsize+guardtime,:,u);% [64,22,4] ⇒ ガードインターバルの除去
    end
else
    for u=1:Num_User
        symbwaves(:,:,u)=reshape(TimeSignal(:,:,u),ifftsize,numsymb);
    end
end
clear TimeSignal;%save memory

% 修正箇所
fftspect=zeros(numsymb,ifftsize,Num_User); % [22,64,4] ※パイロット信号部分のみが一致する可能性がある
for u=1:Num_User
    fftspect(:,:,u)=fft(symbwaves(:,:,u))'./sqrt(ifftsize);% [22,64,4]
end
DataCarriers=fftspect(:,:,:);% [22,64,4] ⇒ FFT,IFFTは2の累乗でないとうまく適用できないので、fftsize=64を用いたが、計算においては冗長
...
H_Res=DataCarriers(1:Num_pilot,:,:);% [2,64,4] ⇒ パイロット信号:チャネル応答（伝播環境）を調べることができる h*x
if Num_pilot==1
   H_Resp=H_Res;
else
...
Noise_power1=abs(DataCarriers(1,:,:)-H_Resp(1,:,:)).^2;
Noise_power2=abs(DataCarriers(2,:,:)-H_Resp(1,:,:)).^2;
...
if Doppler==0
   ...
   %%%ORC  
   %conj：複素共役
   data_faded1=zeros(Num_sym,NumCarr,Num_User); % [20,64,4]
   for u=1:Num_User
       data_faded1(:,:,u)=(DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,u).*conj(repmat(H_Resp(:,:,u),Num_sym,1)))./...
           (abs(repmat(H_Resp(:,:,u),Num_sym,1)).^2+repmat(Noise_power(:,:,u),Num_sym,1)); %%MMSEC　⇒　MIMOへの拡張が難しそう。。。
   end
...
if Doppler==0
    for u=1:Num_User
        for fscb=1:proc_gain_block
            for fsc=1:proc_gain
                data_faded(:,(fscb-1)*proc_gain+fsc,u)=sum(data_faded1(:,(fscb-1)*proc_gain+1:fscb*proc_gain,u).*...
    ...
    for u=1:Num_User
        for fscb=1:proc_gain_block% 1～1
            for fsc=1:proc_gain% 1～64
                ...
                % sum(〇,2) ⇒ 行方向の合計値を求める ⇒ 信号のみを取り出すことができる。[20,64,4]
            end
        end
    end
end

data_faded_final=zeros(Num_sym,NumCarr);
for fscb=1:proc_gain_block% 1～4
    for fsc=1:proc_gain% 1～16
        data_faded_final(:,(fscb-1)*proc_gain+1:fscb*proc_gain)=data_faded(:,(fscb-1)*proc_gain+1:fscb*proc_gain,fscb);
    end
end
