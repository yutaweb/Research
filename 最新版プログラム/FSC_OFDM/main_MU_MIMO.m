...
ifftsize=64;
guardtype=2;% 1:ゼロでうめる 2:サブキャリアの後方部を前方にコピーする
...
Transrate=20*10^6; % Multipath fadingで用いる
...
for l = 1:NumSizes % 1
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
	  ...
	  for r = 1:rep % 1～100 ⇒ 各々のEb/Noに対して100回シミュレーションする
        ...
        Datatx=transmitter(Gen_data,Num_sym,wordsize,NumCarr,guardtype,guardtime,...
        ...
        if guardtype~=0% 等しくない場合は1を返し、等しい場合は0を返す。
            if guardtype == 1%guard period is just a zero transmission
              BaseSignal=[zeros(guardtime,Num_sym+Num_pilot);BaseSignal];
        ...
        % 以下、ユーザ数分だけ作る
        OutSignal1=zeros(1,(Num_sym+Num_pilot)*(NumCarr+guardtime),Num_User); % [1,22*80,4]
        OutSignal=zeros(1,(Num_sym+Num_pilot)*(NumCarr+guardtime),Num_User); % [1,22*80,4]
        Signal_dis1=zeros(1,Num_User); % [1,4]
        Signal_dis=zeros(1,Num_User); % [1,4]
        Fading1=zeros(MultiNo,Num_sym+Num_pilot,Num_User); % [7,22,4]
        Fading=zeros(MultiNo,Num_sym+Num_pilot,Num_User); % [7,22,4]
        for u=1:Num_User % 1～4 ⇒ 各ユーザごとに異なる伝搬環境及び受信アンテナノイズを持っていることに起因する
            [OutSignal(:,:,u),Signal_dis(1,u), Fading(:,:,u)]=channel_only(BaseSignal,NormMulti,MultiNo,...
                Transrate,Doppler,Delay,SNR,com_delay);% channel_only([80,22,2],[7,1],7,1000,4,2,7,0)
            % OutSignal(:,:,u) [1,1760,4]
            % Signal_dis(1,u); % [1,4]
            % Fading(:,:,u); % [7,22,4]
        end   
        ...
        %%  AWGN channel
        TimeSignal=zeros(1,(Num_sym+Num_pilot)*(NumCarr+guardtime),Num_User); % [1,22*80,4]
        % 全ユーザの受信アンテナ数の合計は4本なので、それぞれに異なるノイズを加える必要がある
        for u=1:Num_User
            TimeSignal(1,:,u)=awgn(OutSignal(:,:,u),Signal_dis(1,u),MultiNo,Doppler,SNR,Num_sym,...
                NumCarr,guardtime,Num_pilot,coding_rate,perfect,wordsize);% [[1,22*80,4],1,7,4,7,20,64,16,2,cording_rate,0,1]
        end
        % ⇒ 白色雑音を乗せる。[1,22*80]
        ...
        % Rx device
        %**************
        [Datarx,Datarx_hd]=receiver_MU_MIMO(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                    guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_User);% [[1,22*80],64,64,1,2,16,20,2,4,64]
