...
% TxSignal=[80,22],Multipath=[7,1],MultiNo=7,Transrate=1000,Doppler=4,Delay=2,SNR=7,com_delay=0
%*******************************************************************
...
if Doppler ~= 0% Dopplerが0でなければ、1を0であれば、0を返す。⇒ 今回はDoppler=4
   ...
   for DD=1:MultiNo% 1～7 ⇒ マルチパス分だけフェーディングさせる
     ...
     randtime = 1000*rand;
     N_0   = 32;%16;%Number of ocsillator
     N = 4.0 * N_0;
   ...
   %****************************
   if MultiNo==1 
      fr=repmat(r(:,1:Numsymb),FreqNo,1);%Make fading replica 
