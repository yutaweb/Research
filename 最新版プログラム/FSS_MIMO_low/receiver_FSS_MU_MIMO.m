function [Datarx,Datarx_hd]=receiver(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                     guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_Tx,Num_Rx,Num_User,noise)
...
%         for i=1:Num_Rx % 1～2
%             fftspect(:,(fscb-1)*proc_gain+1:fscb*proc_gain,i,u)=fft(symbwaves((fscb-1)*proc_gain+1:fscb*proc_gain,:,i,u))'./sqrt(proc_gain);
%         end
%     end
% end
% DataCarriers=fftspect(:,:,:,:);% [22,64,2,4] 
% disp(DataCarriers);
% size(DataCarriers)
...
for u=1:Num_User
    ...
    H_m(:,:,:,u)=[h1_1(:,:,:,u) h1_2(:,:,:,u);h2_1(:,:,:,u) h2_2(:,:,:,u)];% [2,2,64,4] ← MIMOにおいてはパス間の相関が全然ないので、 
end
...
% data_faded_final(:,1:16,:)=data_faded(:,1:16,:,1);
