...
noise_power_mean1=mean(abs(noise(:,:,1)));
noise_power_mean2=mean(abs(noise(:,:,2)));
noise_power_mean_both=[noise_power_mean1 0;0 noise_power_mean2];
%disp(noise_power_mean_both);
...
if Doppler==0
   ...
   for i=1:Num_sym % 1～20
         for j=1:NumCarr % 1～64 
           data_faded1(i,j,:)=H_m_resp(:,:,j)'*inv(H_m_resp(:,:,j)'*H_m_resp(:,:,j)+noise_power_mean_both)*(DataCarriers1(:,i,j));    
