function [OutSignal,Signal_dis, Fading]=channel_only_MIMO_OFDMA(TxSignal,Multipath,MultiNo,Transrate,...
    Doppler,Delay,SNR, com_delay,channel_rand)
...
if Doppler ~= 0% Dopplerが0でなければ、1を0であれば、0を返す。⇒ 今回はDoppler=4
   ...
   for DD=1:MultiNo% 1～7 ⇒ マルチパス分だけフェーディングさせる
     ...
     randpoint = channel_rand(:,:,:,1,DD,:,:);% 0～1の間をランダムにとる
     randtime = 1000*channel_rand(:,:,:,2,DD,:,:);
