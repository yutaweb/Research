% ユーザ毎に異なる拡散符号を割り当てる手法を考える。（意味ある？）
% 従来手法である、電力割り当てによる手法を評価する
% OFDMAでは、完全に分離できている前提ではなく、SICなどで分離してユーザー間干渉を考慮した上で、評価する必要がある。（調）
% 高次元変調に対応させる 
% ※とりあえずは、16ビットで行う
% ※4G：64ビット（64QAM）
% ※5G：256ビット（256QAM） Wifi6の規格である802.11axでは256サブキャリアを用いている
% ※1024QAMまで対応させる必要がある
% アンテナの本数を変えて評価する
% スループットの評価をする
%
