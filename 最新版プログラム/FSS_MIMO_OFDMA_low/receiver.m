...
% TimeSignal=[1,22*80,2],ifftsize=64,carriers=[1,64],wordsize=1,guardtype=2,guardtime=16,Num_sym=20,Num_pilot=2,Doppler=4,proc_gain=64
% Num_Tx=2,Num_Rx=2
%****************************************************************************    
...
for receive=1:Num_Rx
    fftspect(:,:,receive)=fft(symbwaves(:,:,receive))';% [22,64,2]
end
DataCarriers=fftspect(:,carriers,:);% [22,64,2] ⇒ FFT,IFFTは2の累乗でないとうまく適用できないので、fftsize=64を用いたが、計算においては冗長
...
H_m=[h1_1 h1_2;h2_1 h2_2];% [2,2,64]
...
% プログラム的に少し怪しいが、後で修正する。
disp(Noise_power); % ノイズの量がばけもん
...
if Doppler==0
   ...
   %            (abs(repmat(H_Resp,Num_sym,1)).^2+repmat(Noise_power,Num_sym,1)); %%MMSEC ⇒ OFDMの時とは
   % [20,64,2].*[20,64]./([20,64]+[20,64])
   DataCarriers1=permute(DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,:),[3 1 2]);%[2 20 64];
   for i=1:NumCarr % 1～64
        for j=1:Num_sym % 1～20
           data_faded1=conj(H_m_resp(:,:,i))./((H_m_resp(:,:,i)'.*H_m_resp(:,:,i))+Noise_power())*(DataCarriers1(:,j,i));      
        end   
   end
end
...
if Doppler==0
    ...
    for fscb=1:proc_gain_block% 1～1
      for fsc=1:proc_gain% 1～64
         ...
         % sum(〇,2) ⇒ 行方向の合計値を求める ⇒ 信号のみを取り出すことができる。[20,64]
      end
   end
end
