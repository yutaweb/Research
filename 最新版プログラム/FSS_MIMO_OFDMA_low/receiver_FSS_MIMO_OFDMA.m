...
for fsc=1:size(user_index,2)% 1～2
    for fscb=1:size(user_index,1)% 1～4
        data_faded_final(:,(channel_ranking(fscb,fsc)-1)*proc_gain+1:channel_ranking(fscb,fsc)*proc_gain,fsc)=data_faded(:,(channel_ranking(fscb,fsc)-1)*proc_gain+1:channel_ranking(fscb,fsc)*proc_gain,fsc,user_index(fscb,fsc));
    end
end
...
for i=1:Num_Rx
    for  k = 1:Num_sym % 1:20
       for cc=1:NumCarr % 1:64
          ...
          elseif wordsize==4
             X(k,cc,i)=data_faded_final(k,cc,i)*sqrt(10); % [20,64,1～2]
