function [TimeSignal,noise]=awgn(OutSignal,Signal_dis,MultiNo,Doppler,SNR,Num_sym,...
                         NumCarr,guardtime,Num_pilot,coding_rate,perfect,wordsize,k,r,user,receive,noise_rand)
%*******************************************************************************         
...
% rng('default');
% rng(20);
% rng((r-1)*100+(k-1)*10+(receive-1)*3+user);
% disp((r-1)*100+(k-1)*10+(receive-1)*3+user)
...
%    noise=sqrt(1/2)*(randn(1,length(OutSignal))*(i)+randn(1,length(OutSignal))).*(NoiseFac);%
   noise=sqrt(1/2)*(noise_rand(1,:,receive,user,1,k,r)*(i)+noise_rand(1,:,receive,user,2,k,r)).*(NoiseFac);%
end
