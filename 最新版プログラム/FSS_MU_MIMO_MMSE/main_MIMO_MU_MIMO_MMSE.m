...
rep=500;%Number of loop times ⇒　任意の回数ループする（シミュレーション回数）
...
for l = 1:NumSizes % 1
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
      ...
      for r = 1:rep % 1～10 ⇒ 各々のEb/Noに対して任意の回数シミュレーションする
        ...
        %% transmitter
         Datatx=transmitter_MU_MIMO(Gen_data,Num_sym,wordsize,NumCarr,guardtype,guardtime,...
...
semilogy(SNRMin+10*log10(coding_rate):SNRInc:SNRMax+10*log10(coding_rate),Result(:,2)','b>--',...
