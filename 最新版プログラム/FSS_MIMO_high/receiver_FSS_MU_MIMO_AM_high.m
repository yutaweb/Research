function [Datarx,Datarx_hd]=receiver(TimeSignal,ifftsize,carriers,wordsize,guardtype,...
                                     guardtime,Num_sym,Num_pilot,Doppler,proc_gain,Num_Tx,Num_Rx,Num_User,noise,channel_ranking,user_index,block_num_per_user)
% TimeSignal=[1,22*80,2,4],ifftsize=64,carriers=[1,64],wordsize=2,guardtype=2,guardtime=16,Num_sym=20,Num_pilot=2,Doppler=4,proc_gain=16,noise=[1,80*22,2,4]
% channel_ranking=[4,2] → [:,1]には、送信アンテナ1から送信されるべきストリームの割り当てが入っている
%                          [:,2]には、送信アンテナ2から送信されるべきストリームの割り当てが入っている
% user_index=[4,2] → [:,1]には、送信アンテナ1から送信されるべきストリームのユーザ番号が入っている
%                     [:,2]には、送信アンテナ2から送信されるべきストリームのユーザ番号が入っている
% Num_Tx=2,Num_Rx=2
%****************************************************************************    
...
% 全サブキャリアに一気にIFFTを掛ける場合
fftspect=zeros(numsymb,ifftsize,Num_Rx,Num_User); % [22,64,2,4] ※パイロット信号部分のみが一致する可能性がある
for u=1:Num_User
    for receive=1:Num_Rx
...
%*******************************
% Estimate the channel response それぞれのユーザでチャネル推定をする必要がある
%*******************************
...
for u=1:Num_User
    ...
    h2_2(:,:,:,u)=permute(H_Res(2,:,2,u),[1 3 2 4]);% h1'-h2+n2 '[1,1,64,4]
    H_m(:,:,:,u)=[h1_1(:,:,:,u) h1_2(:,:,:,u);h2_1(:,:,:,u) h2_2(:,:,:,u)];% [2,2,64,4] 
end
...
noise_std1=zeros(1,Num_User); % [1,4]
noise_std2=zeros(1,Num_User); % [1,4]
noise_std=zeros(Num_Rx,Num_Tx,Num_User); % [2,2,4]
gause_noise=zeros(Num_Rx,Num_Tx,Num_User); % [2,2,4]
for u=1:Num_User % 1～4 ※受信アンテナごとにノイズは異なるので、各アンテナにおけるノイズ分散を算出している
    noise_std1(1,u)=(std(noise(1,:,1,u)))^2; % [1,4]
    noise_std2(1,u)=(std(noise(1,:,2,u)))^2; % [1,4]
    noise_std(:,:,u)=[noise_std1(1,u) 0;0 noise_std2(1,u)]; % [2,2,4]
    gause_noise(:,:,u)=Num_Tx*noise_std(:,:,u).*eye(Num_Rx); % [2,2,4]
end

if Doppler==0
    data_faded1=DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,:); % [20,64,2,4]  
else
   
   DataCarriers1=permute(DataCarriers(Num_pilot+1:Num_pilot+Num_sym,:,:,:),[3 1 2 4]);%[2 20 64 4];
   % ココでの計算は、data_faded1が[20,64,2]の状態を保ったまま 
   data_faded1=zeros(Num_sym,NumCarr,Num_Rx,Num_User); % [20,64,2,4]
...
proc_gain=proc_gain/block_num_per_user;
proc_gain_block=proc_gain_block*block_num_per_user;
spread=hadamard(proc_gain);% [16,16]
        ...
        for i=1:Num_Rx
            for fscb=1:proc_gain_block% 1～4
                for fsc=1:proc_gain% 1～16
                    ...
                    % ユーザ１：[20,64,2,1]の[20,1 ～16,2,1]
                    % ユーザ２：[20,64,2,2]の[20,17～32,2,2]
                    % ユーザ３：[20,64,2,3]の[20,33～48,2,3]
                    % ユーザ４：[20,64,2,4]の[20,49～64,2,4]
                    % [20,64,2,4]
                end
            end
        end
...
for fsc=1:size(user_index,2)% 1～2
    for fscb=1:size(user_index,1)% 1～4
        for b=1:block_num_per_user
            data_faded_final(:,(channel_ranking(fscb,b,fsc)-1)*proc_gain+1:channel_ranking(fscb,b,fsc)*proc_gain,fsc)=data_faded(:,(channel_ranking(fscb,b,fsc)-1)*proc_gain+1:channel_ranking(fscb,b,fsc)*proc_gain,fsc,user_index(fscb,fsc));
        end
%         data_faded_final(:,(channel_ranking(fscb,1,fsc)-1)*proc_gain+1:channel_ranking(fscb,1,fsc)*proc_gain,fsc)=data_faded(:,(channel_ranking(fscb,1,fsc)-1)*proc_gain+1:channel_ranking(fscb,1,fsc)*proc_gain,fsc,user_index(fscb,fsc));
%         data_faded_final(:,(channel_ranking(fscb,2,fsc)-1)*proc_gain+1:channel_ranking(fscb,2,fsc)*proc_gain,fsc)=data_faded(:,(channel_ranking(fscb,2,fsc)-1)*proc_gain+1:channel_ranking(fscb,2,fsc)*proc_gain,fsc,user_index(fscb,fsc));
    end
end
