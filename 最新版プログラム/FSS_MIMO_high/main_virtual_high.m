function [channel_ranking,user_index]=main_virtual_high(SNR,Doppler,com_delay,ifftsize,guardtype,guardtime,...
    MultiNo,Transrate,Delay,ww,Num_sym,NumCarr,Num_pilot,strength_len,...
    carriers,proc_gain,WordSizes,NumSizes,k,Num_Tx,Num_Rx,Num_User,channel_rand,r,block_num_per_user)
...
for l = 1:NumSizes % 1
    ...
    %   channel ranking
    %************************
    % high granularityの際、サブキャリアの分割数を増やしたい
    user=zeros(Num_User,Num_User*block_num_per_user,Num_Tx); % [4,4,2]
    proc_gain_block=NumCarr/proc_gain*block_num_per_user;
...
%     end 
    ...
    user_mean=zeros(Num_User,Num_Rx); % [4,2]
    ...
    M_min=zeros(Num_User,1,Num_Tx); % [4,1,2]
    I_min=zeros(Num_User,1,Num_Tx); % [4,1,2]
    M_max=zeros(Num_User,block_num_per_user,Num_Tx); 
    I_max=zeros(Num_User,block_num_per_user,Num_Tx); 
    user_index=zeros(Num_User,Num_Tx); % [4,2]
    channel_ranking=zeros(Num_User,block_num_per_user,Num_Tx); % [4,2,2]
    
    % 各ユーザが一気に最適なサブキャリアブロックを選択する場合
    % 例）4user, 8分割の際は, 2ブロックを一気に選択する
%     for i=1:Num_Tx % 1～2
%         for u=1:Num_User % 1～4
%             [M_min(u,1,i),I_min(u,1,i)]=min(user_mean(:,i));
%             user_index(u,i)=I_min(u,1,i);
%             
% %             ループを使わないで記述する方法
% %             [M_max(u,1,i),I_max(u,1,i)]=max(user(I_min(u,1,i),:,i));
% %             channel_ranking(u,1,i)=I_max(u,1,i);
% %             % 評価し終わったら、ゼロ又は大きな数で埋める
% %             user(:,I_max(u,1,i),i)=zeros(Num_User,1);
% %             
% %             [M_max(u,2,i),I_max(u,2,i)]=max(user(I_min(u,1,i),:,i));
% %             channel_ranking(u,2,i)=I_max(u,2,i);
% %             user(:,I_max(u,2,i),i)=zeros(Num_User,1);
% 
%             for b=1:block_num_per_user
%                 [M_max(u,b,i),I_max(u,b,i)]=max(user(I_min(u,1,i),:,i));
%                 channel_ranking(u,b,i)=I_max(u,b,i);
%                 % 評価し終わったら、ゼロ又は大きな数で埋める
%                 user(:,I_max(u,b,i),i)=zeros(Num_User,1);
%             end
%             
%             user(I_min(u,1,i),:,i)=zeros(1,Num_User*block_num_per_user);
%             user_mean(I_min(u,1,i),i)=100;
%         end
%     end   

    % 各ユーザが1つずつ最適なサブキャリアブロックを選択する場合
    % 例）4user, 8分割の際は, 各ユーザが1ブロックずつ最適なブロックを選択したすることを繰り返す
    for i=1:Num_Tx % 1～2
        for u=1:Num_User % 1～4
            [M_min(u,1,i),I_min(u,1,i)]=min(user_mean(:,i));
            user_index(u,i)=I_min(u,1,i);
