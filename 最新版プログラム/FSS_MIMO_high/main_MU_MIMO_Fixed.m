...
%*****************
% 課題
% ①2×2MIMOを用いる場合、ユーザ数Num_Userとproc_gainを変更するだけで、良いプログラムにする
% ②その他機能をまとめる
% ③SISOと4×4MIMOについては時間があれば、検討する
%*****************
%******************
% Simulation start
%******************
...
% 8,16ユーザへの拡張
%********************
%proc_gain=8; %OFDMAによって、 8ユーザに割り当てる
%proc_gain=4; %OFDMAによって、16ユーザに割り当てる
% 4 ユーザに割り当てるときは、proc_gain:16
% 8 ユーザに割り当てるときは、proc_gain:8
% 16ユーザに割り当てるときは、proc_gain:4
...
for l = 1:NumSizes % 1
   ...
   for k = 1:((SNRMax-SNRMin)/SNRInc)+1 % 1～7 
	  ...
	  for r = 1:rep % 1～100 ⇒ 各々のEb/Noに対して任意の回数シミュレーションする
        ...
        %*********************
        % Tx device ⇒ 送信機
        % OFDMAの場合、各ストリームからは、同じストリームを送信し、受信側で自分に割り当てられているデータを取り出す。
        % ①チャネル数を1として、各送信アンテナからストリームを送信する。
        % ②チャネル数を増やし、格差う新アンテナからストリームを送信する。
        %*********************
        if strength_len==0
...
%         end
        ...
        %*****************
        %  Channel block ⇒ ユーザの数を考慮する必要があることに注意する
        %*****************
        ...
        for u=1:Num_User % 1～4 ⇒ 各ユーザごとに異なる伝搬環境及び受信アンテナノイズを持っていることに起因する
            for i=1:Num_Rx % 1～2
                for j=1:Num_Tx % 1～2 Signal_disは後で改善する ⇒ ただデータを保存してるだけなので、Fadingに直接の影響はない。
                    [OutSignal1(:,:,j,u),Signal_dis1(j,u), Fading1(:,:,j,u)]=channel_only(BaseSignal(:,:,j),NormMulti,MultiNo,...
