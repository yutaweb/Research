...
rng('default');
rng(10); % ランダムの値を変更したいときは、この数字を変更する
% 条件を統一させるために先にすべてのランダム値を生成する
channel_rand=rand([Num_Tx,Num_Rx,Num_User,2,MultiNo,(SNRMax-SNRMin)/SNRInc+1,rep]); 
% [2,2,4,7,7,rep] → [送信アンテナ数,受信アンテナ数,ユーザー数,channel_only内での使用数,チャネル伝播路数,マルチパス数,SNR反復数,全反復数]
...
%         end
        ...
        if guardtype~=0% 等しくない場合は1を返し、等しい場合は0を返す。
            if guardtype == 1%guard period is just a zero transmissionリー元
